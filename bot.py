"""
一个基于 Telegram 的 OpenAI 聊天机器人。

工作流程：
1. 用户在 Telegram 里发消息给机器人
2. python-telegram-bot 库收到消息，触发我们写的 handler 函数
3. handler 把消息转发给 OpenAI API
4. 拿到 OpenAI 的回复后，发回给用户
...
if not TELEGRAM_BOT_TOKEN or not OPENAI_API_KEY:
    raise ValueError(
        "请先在 .env 文件里配置 TELEGRAM_BOT_TOKEN 和 OPENAI_API_KEY，"
        "可以参考 .env.example"
    )
...
# OpenAI API 客户端
client = OpenAI(base_url=OPENAI_BASE_URL or None, api_key=OPENAI_API_KEY)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    处理用户发来的普通文本消息：
    1. 把消息加入这个用户的历史
    2. 调用 OpenAI API，把历史一起传过去（这样它才有"记忆"）
