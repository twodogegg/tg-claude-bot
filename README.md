# Telegram Claude 聊天机器人

一个接入 Claude API 的 Telegram 智能聊天机器人，用来练习 Python + AI API 调用 + 异步编程。

## 第一步：拿到两个必需的密钥

### 1. Telegram Bot Token
1. 在 Telegram 里搜索 `@BotFather`
2. 发送 `/newbot`，按提示起个名字（比如 `MyClaudeBot`）
3. 完成后 BotFather 会给你一串 token，形如 `123456789:ABCdefGHIjklMNOpqrsTUVwxyz`
4. 复制保存好

### 2. Anthropic API Key
1. 打开 https://console.anthropic.com/
2. 注册/登录后，进入左侧 "API Keys"
...
1. **初始化配置**：读取 .env 里的密钥，创建 Claude 客户端
2. **对话历史存储**：用一个字典 `{用户ID: [消息列表]}` 给每个用户维护独立的聊天记录
3. **命令处理**：`/start`（欢迎语）、`/reset`（清空历史）
4. **核心消息处理**：收到消息 → 存入历史 → 调用 Claude API → 把回复发回去
5. **启动**：注册所有 handler，开始轮询 Telegram 服务器

## 常见问题

- **机器人不回复**：检查 `.env` 里的 token 是否正确，看终端日志有没有报错
- **报 "Invalid API Key"**：Anthropic API Key 填错了，或者账户没有余额
- **想换成流式回复（打字机效果）**：可以研究 `claude.messages.stream()`，这是进阶练习
...
3. 加一个 `/model` 命令，让用户可以切换不同的 Claude 模型
